function [] = Zplot()
%Zplot Calculates Z for various P and T from composition input, assuming gas
%of C1,C2,C3 only. 
...
Z = zeros(4,21);  % Preallocate matrix, where the four rows correspond to four different temperatures, 50/100/150/200 F
for i=1:1:4
    % Effectively i sets working temperature and j working pressure
    % A,C,D are functions of Temperature alone and therefore are calculated in the i-loop
    A=1.39*(Tpr(i)-0.92)^0.5-0.36*Tpr(i)-0.101;
    C=0.132-0.32*log10(Tpr(i));
    D=10^(0.3106-0.49*Tpr(i)+0.1824*Tpr(i)^2);
    for j=1:1:21
        B=(0.62-0.23*Tpr(i))*Ppr(j)+(0.066/(Tpr(i)-0.86)-0.037)*Ppr(j)^2+0.32/(10^(9*(Tpr(i)-1)))*Ppr(j)^6;
        Z(i,j)=A+(1-A)/exp(B)+C*Ppr(j)^D;
    end
end
